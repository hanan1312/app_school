...
import type { ConfigLookupCategory, ConfigLookupItem } from "../lib/types";
...
export default function ConfigLookupListModal({ category, title, icon: Icon, onClose }: Props) {
  const { token } = useAuth();
  const [items, setItems] = useState<ConfigLookupItem[]>([]);
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.getConfigLookup(token, category);
      setItems(res.items);
    } catch {
  ...
  const startEdit = (item: ConfigLookupItem) => {
    setName(item.name);
  ...
  const save = async () => {
    ...
    try {
      if (editingId === "new") {
        await api.createConfigLookup(token, { category, name: name.trim() });
      } else if (typeof editingId === "number") {
        await api.updateConfigLookup(token, editingId, { name: name.trim() });
      }
      setEditingId(null);
      await load();
    } catch (err) {
  ...
  const remove = async (id: number) => {
    ...
    try {
      await api.deleteConfigLookup(token, id);
      await load();
    } catch {
