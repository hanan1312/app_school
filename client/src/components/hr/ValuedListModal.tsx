...
import type { HrValuedCategory, HrValuedItem } from "../../lib/types";
...
export default function ValuedListModal({ category, title, icon: Icon, amountLabel = "Amount", onClose }: Props) {
  const { token } = useAuth();
  const { selectedSchoolId } = useSchools();
  const [items, setItems] = useState<HrValuedItem[]>([]);
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.getHrValued(token, category, selectedSchoolId ?? undefined);
      setItems(res.items);
    } catch {
  ...
  }, [token, category, selectedSchoolId]);
  ...
  const startEdit = (item: HrValuedItem) => {
    setName(item.name);
  ...
  const save = async () => {
    ...
    try {
      if (editingId === "new") {
        await api.createHrValued(token, { category, name: name.trim(), amount: numAmount, isPercentage });
      } else if (typeof editingId === "number") {
        await api.updateHrValued(token, editingId, { name: name.trim(), amount: numAmount, isPercentage });
      }
      setEditingId(null);
      await load();
    } catch (err) {
  ...
  const formatAmount = (item: HrValuedItem) => (item.is_percentage ? `${item.amount}%` : item.amount);
