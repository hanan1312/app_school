...
import type { HrLookupCategory, HrLookupItem } from "../../lib/types";
...
export default function LookupListModal({ category, title, icon: Icon, perSchool, withNote, onClose }: Props) {
  const { token } = useAuth();
  const { selectedSchoolId } = useSchools();
  const [items, setItems] = useState<HrLookupItem[]>([]);
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.getHrLookup(token, category, selectedSchoolId ?? undefined);
      setItems(res.items);
    } catch {
  ...
  const startEdit = (item: HrLookupItem) => {
    setName(item.name);
    setNote(item.note ?? "");
  ...
  const save = async () => {
    ...
    try {
      if (editingId === "new") {
        await api.createHrLookup(token, {
          category,
          name: name.trim(),
          note: withNote ? note.trim() || undefined : undefined,
          schoolId: perSchool ? selectedSchoolId ?? undefined : undefined,
        });
      } else if (typeof editingId === "number") {
        await api.updateHrLookup(token, editingId, { name: name.trim(), note: withNote ? note.trim() : undefined });
      }
      setEditingId(null);
      await load();
    } catch (err) {
  ...
  const remove = async (id: number) => {
    ...
    try {
      await api.deleteHrLookup(token, id);
      await load();
    } catch {
